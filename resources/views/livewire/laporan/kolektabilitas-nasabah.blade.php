<section>
    ...
    <div class="flex gap-14">
        {{-- head sisi kiri --}}
        <div class="w-4/6">
            <h1 class="text-3xl font-bold">Laporan Kolektibilitas Nasabah</h1>
            <div class="breadcumb">
                <a href="{{ route('dashboard') }}">Dasboard</a>
                <span>Laporan Kolektibilitas Nasabah</span>
            </div>
        </div>
    ...
    <article class="flex mt-5">
        ...
        <div class="mr-1 md:mr-2 lg:mr-3 w-4/6">
            {{-- grafik Nasabah combine Kolektibilitas(tanpa pilih Kolektibilitas), kolek, pekerja, tim(pilih Kolektibilitas) --}}
            <div class="bg-gray-900 my-2 md:my-4 lg:my-6 px-6 py-3 rounded-md shadow">
                <div class="mb-4 pb-1 flex">
                    <div class="w-screen flex items-center">
                        <h4 class="text-base font-thin text-gray-400">Grafik Kolektibilitas
                    ...
                    <div class="w-full justify-end flex items-center cursor-pointer" wire:click="showChartModal">
                        ...
                        <span
                            class="rounded-r-md py-1 px-2 pl-0 bg-indigo-700 text-white font-thin text-xs items-center">{{ $pickRelationId ? ucwords(App\Models\Kolektibilitas::find($pickRelationId)->singkatan) . ',' : '' }}
                            Per{{ $opsRange }}</span>
        ...
        <div class="ml-1 md:ml-2 lg:ml-3 w-2/6">
            ...
            <div class="my-2 md:my-4 lg:my-6 py-3 px-6 text-sm bg-gray-900 shadow rounded-md">
                <div class="pb-1">
                    <h4 class="text-base font-thin text-gray-400">Presentase
                        {{ $pickRelationId ? ($opsRange == $rangeList[0] ? 'Pertahun' : 'Perbulan') : 'Kolektibilitas' }}</h4>
                </div>
    ...
    <x-jet-dialog-modal wire:key="modelChar" wire:model="modalChartFormVisible">
        <x-slot name="title">
            {{ __('Pengaturan Grafik Kolektibilitas') }}
        </x-slot>
        ...
        <x-slot name="content">
            <div wire:ignore
                    ...
                    <div x-show="!isRegions">
                        ...
                        <div class="flex">
                            <label wire:click="chartChange" @click="monthsOrYears='{{ $rangeList[1] }}'">
                                <input wire:model="opsRange" {{ $rangeList[1] == $opsRange ? 'checked="true"' : '' }}
                                    type="radio" value="{{ $rangeList[1] }}" name="range"
                                    wire:key="range_{{ $rangeList[1] }}" class="first option_round">
                            ...
                            <label wire:click="chartChange" @click="monthsOrYears='{{ $rangeList[0] }}'">
                                <input wire:model="opsRange" {{ $rangeList[0] == $opsRange ? 'checked="true"' : '' }}
                                    type="radio" value="{{ $rangeList[0] }}" name="range"
                                    wire:key="range_{{ $rangeList[0] }}" class="last option_round">
                ...
                <div>
                    ...
                    @foreach (collect($relationData)->chunk(6)->all()
    as $z => $data)
                        <div class="flex pt-1">
                            ...
                            @if ($z == 0)
                                <label wire:click="chartChange" @click="isRegions=false">
                                    ...
                                    <input wire:model="pickRelationId" {{ !$pickRelationId ? 'checked="true"' : '' }}
                                        type="radio" value="0" name="scope" wire:key="scope_0"
                            ...
                            @foreach ($data as $i => $item)
                                <label wire:click="chartChange" {{-- @click="isRegions=true;document.querySelector('#reset_compare').children[0].click();" --}}>
                                    <input wire:model="pickRelationId" type="radio" value="{{ $item->id }}"
                                        name="scope" wire:key="scope_{{ $i }}"
                                        class="{{ $z > 0 && $i == 0 ? 'first' : ($i + 1 == count($data) ? 'last' : '') }} option_round">
