<section>
    ...
    <div class="flex gap-14">
        {{-- head sisi kiri --}}
        <div class="w-4/6">
            <h1 class="text-3xl font-bold">Laporan Penghasilan Nasabah</h1>
            <div class="breadcumb">
                <a href="{{ route('dashboard') }}">Dasboard</a>
                <span>Laporan Penghasilan Nasabah</span>
            </div>
        </div>
     ...
     <article class="flex mt-5">
        ...
        <div class="mr-1 md:mr-2 lg:mr-3 w-4/6">
              {{-- grafik Nasabah combine Penghasilan(tanpa pilih Penghasilan), kolek, pekerja, tim(pilih Penghasilan) --}}
              <div class="bg-gray-900 my-2 md:my-4 lg:my-6 px-6 py-3 rounded-md shadow">
                <div class="mb-4 pb-1 flex">
                    <div class="w-screen flex items-center">
                        <h4 class="text-base font-thin text-gray-400">Grafik Penghasilan
                    ...
                    >
                        ...
                        <span
                            class="rounded-r-md py-1 px-2 pl-0 bg-indigo-700 text-white font-thin text-xs items-center">{{ $pickRelationId ? ucwords(App\Models\Penghasilan::find($pickRelationId)->nama) . ',' : '' }}
                            Per{{  $opsRange }}</span>
        ...
        <div class="ml-1 md:ml-2 lg:ml-3 w-2/6">
            ...
            <div class="my-2 md:my-4 lg:my-6 py-3 px-6 text-sm bg-gray-900 shadow rounded-md">
                <div class="pb-1">
                    <h4 class="text-base font-thin text-gray-400">Presentase {{$pickRelationId?($opsRange=='tahun'?'Pertahun':'Perbulan'):'Penghasilan'}}</h4>
                </div>
    ...
    <x-jet-dialog-modal wire:key="modelChar" wire:model="modalChartFormVisible">
        <x-slot name="title">
            {{ __('Pengaturan Grafik Penghasilan') }}
        </x-slot>
        ...
        <x-slot name="content">
            <div wire:ignore
                ...
                <div>
                    ...
                    @foreach (collect($relationData)->chunk(3)->all()
                    as $z => $data)
                                        <div class="flex pt-1">
