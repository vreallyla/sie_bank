<section>
    ...
    <div class="flex gap-14">
        {{-- head sisi kiri --}}
        <div class="w-4/6">
            <h1 class="text-3xl font-bold">Daftar Kolektibilitas</h1>
            <div class="breadcumb">
                <a href="{{ route('dashboard') }}">Dasboard</a>
                <span>Daftar Kolektibilitas</span>
            </div>
        </div>
                            ...
                            @if ($data->count())
                                @foreach ($data as $item)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 text-sm whitespace-no-wrap">{{ $item->singkatan }}</td>
    ...
    {{ $data->links() }}
     ...
     <x-jet-dialog-modal wire:model="modalFormVisible">
        <x-slot name="title">
            {{ __(($dataId?'Ubah':'Buat') . ' Kolektibilitas') }}
        </x-slot>
        ...
        <x-slot name="content">
            ...
            <div class="mt-4">
                <x-jet-label for="singkatan"  >
                    {{ __('Singkatan') }}<span class="text-base text-gray-400">*</span>
                </x-jet-label>
                <x-jet-input id="singkatan" class="block mt-1 w-full" type="text" placeholder="Isi Singkatan" wire:model.debounce.800ms="singkatan" />
                @error('singkatan') <span class="error">{{ $message }}</span> @enderror
            </div>
    ...
    <x-jet-dialog-modal wire:model="modalConfirmDeleteVisible">
        ...
        <x-slot name="content">
            <span class="text-gray-400">Yakin hapus kolekbilitas?</span>
        </x-slot>
