                        ...
                        @if ($data->count())
                            @foreach ($data as $item)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 text-sm whitespace-no-wrap">{{ $item->alamat_anggunan }}</td>
                                    <td class="px-6 py-4 text-sm whitespace-no-wrap">{{ $item->alamat_instansi }}</td>
    ...
    <x-jet-dialog-modal wire:model="modalFormVisible">
        ...
        <x-slot name="content">
            <div class="mt-4">
                ...
                <x-jet-input id="nik" class="block mt-1 w-full" type="text" placeholder="Isi NIK" wire:model.debounce.800ms="nik" />
            ...
            <div class="mt-4">
                ...
                <x-jet-input id="no_debitur" class="block mt-1 w-full" type="text" placeholder="Isi Nomor Debitur" wire:model.debounce.800ms="no_debitur" />
            ...
            <div class="mt-4">
                ...
                <x-jet-input id="tgl_lahir" class="block mt-1 w-full" type="date" wire:model.debounce.800ms="tgl_lahir" />
            ...
            <div class="mt-4">
                ...
                <x-jet-input id="alamat_anggunan" class="block mt-1 w-full" type="text" placeholder="Isi Alamat Anggunan" wire:model.debounce.800ms="alamat_anggunan" />
            ...
            <div class="mt-4">
                ...
                <x-jet-input id="alamat_instansi" class="block mt-1 w-full" type="text" placeholder="Isi Alamat Instansi" wire:model.debounce.800ms="alamat_instansi" />
...
{{--            </div>--}}
            <div class="mt-4">
                ...
                <select wire:model.debounce.800ms="pekerjaan_id" class="border-white mt-1 w-full" id="pekerjaan_id">
                    ...
                    @foreach ($profesiList as $item)
                        <option value="{{$item->id}}" {{ $item->id==$pekerjaan_id?'selected':'' }}>{{$item->nama}}</option>
                    @endforeach
                </select>
            ...
            <div class="mt-4">
                ...
                <select wire:model.debounce.800ms="penghasilan_id" class="border-white mt-1 w-full" id="penghasilan_id">
                    ...
                    @foreach ($penghasilanList as $item)
                        <option value="{{$item->id}}" {{ $item->id==$penghasilan_id?'selected':'' }}>{{$item->nama}}</option>
                    @endforeach
                </select>
            ...
            <div class="mt-4">
                ...
                <select wire:model.debounce.800ms="pendidikan_id" class="border-white mt-1 w-full" id="pendidikan_id">
                    ...
                    @foreach ($pendidikanList as $item)
                        <option value="{{$item->id}}" {{ $item->id==$pendidikan_id?'selected':'' }}>{{$item->nama}}</option>
                    @endforeach
                </select>
            ...
            <div class="mt-4">
                ...
                <select wire:model.debounce.800ms="kolektibilitas_id" class="border-white mt-1 w-full" id="kolektibilitas_id">
                    <option value="" selected disabled>Pilih Kolektibilitas</option>
                    @foreach ($kolektibilitasList as $item)
                        <option value="{{$item->id}}" {{ $item->id==$kolektibilitas_id?'selected':'' }}>{{$item->nama}}</option>
                    @endforeach
                </select>
            ...
            <div class="mt-4">
                ...
                <select wire:model.debounce.800ms="produk_id" class="border-white mt-1 w-full" id="produk_id">
                    ...
                    @foreach ($produkList as $item)
                        <option value="{{$item->id}}" {{ $item->id==$produk_id?'selected':'' }}>{{$item->nama}}</option>
                    @endforeach
                </select>
