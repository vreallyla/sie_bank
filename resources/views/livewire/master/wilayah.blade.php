<section>
    ...
    <div class="flex gap-14">
        {{-- head sisi kiri --}}
        <div class="w-4/6">
            <h1 class="text-3xl font-bold">Daftar Wilayah</h1>
            <div class="breadcumb">
...
{{--                                    <td class="px-6 py-4 text-sm whitespace-no-wrap">{{ $item->kacap_id }}</td>--}}
                                    <td class="px-6 py-4 text-sm whitespace-no-wrap">{{ $item->telp }}</td>
                                    <td class="px-6 py-4 text-sm whitespace-no-wrap">{{ $item->alamat }}</td>
    ...
    <x-jet-dialog-modal wire:model="modalFormVisible">
        ...
        <x-slot name="content">
            ...
            <div class="mt-4">
                <x-jet-label for="kacap_id">
                    {{ __('Kacab') }}<span class="text-base text-gray-400">*</span>
                </x-jet-label>
                <select wire:model.debounce.800ms="kacap_id" class="border-white mt-1 w-full" id="kacap_id">
                    <option value="" selected disabled>Pilih Kacab</option>
                    @foreach ($kacapList as $item)
                        <option value="{{$item->id}}" {{ $item->id==$kacap_id?'selected':'' }}>{{$item->name}}</option>
                    @endforeach
            ...
            <div class="mt-4">
                <x-jet-label for="alamat"  >
                    {{ __('Alamat') }}<span class="text-base text-gray-400">*</span>
                </x-jet-label>
                <x-jet-input id="alamat" class="block mt-1 w-full" type="text" placeholder="Isi Alamat" wire:model.debounce.800ms="alamat" />
    ...
    <x-jet-dialog-modal wire:model="modalConfirmDeleteVisible">
        ...
        <x-slot name="content">
            <span class="text-gray-400">Yakin hapus Wilayah?</span>
        </x-slot>
